...
from get_point_group_rotmat_twostep import rot_glb_dets, rot_glb_all
...
if __name__ == '__main__':
    ...
    nsymm_for_aver = nsymm

    # Tinv itself is not a magnetic opertor . One needs two steps!! 
    delta_T = np.float64(sys.argv[1]) 

    if delta_T==1.0:
        line=" For magnetic group, part-I, no Time inversion (unprimed), delta(h)=1. see magmom_unprimed.log"
        FL=open("magmom_unprimed.log","w")
        print>>FL, line; print line
    elif delta_T==-1.0:
        line=" For magnetic group, part-II, with Time inversion (primed), delta(h)=-1. see magmom_primed.log"
        FL=open("magmom_primed.log","w")
        print>>FL, line; print line
    else:
        print "delta_T can only be 1.0(unprimed) or -1.0(primed) !"

    print " isymm,  magdiff, MagSymop? "
    for isymm in range(nsymm):         
        magdiff=np.zeros((3),dtype=np.float64)
        for iptr in range(nptrans):
            print>>FL, "----------------isymm iptr ", isymm+1, iptr
            print>>FL, "rot_glb \n", rot_glb_all[isymm]
            print>>FL, ""
            for ii in range(natom_wan):
                rotmap[ii]=wann_atom_rotmap[isymm][iptr][ii]
                print>>FL, "atom ii ", ii+1, "M_ii", atoms_magmom[:,ii], "=rot to=> atom jj ", rotmap[ii]+1, "M_jj", atoms_magmom[:,rotmap[ii]]
                magmom_new = rot_glb_dets[isymm]*delta_T*np.dot(rot_glb_all[isymm],atoms_magmom[:,ii])
                print>>FL, "det(h)*delta*Rot_glb*M_ii", magmom_new,  "M_jj", atoms_magmom[:,rotmap[ii]]
                print>>FL, "diff ",  magmom_new - atoms_magmom[:,rotmap[ii]]
                print>>FL, ""
                magdiff = magdiff+ abs(magmom_new - atoms_magmom[:,rotmap[ii]])
        print>>FL, "MagSymop ?", sum(abs(magdiff))<1.0E-6
        print>>FL, ""

        print isymm+1, magdiff, sum(abs(magdiff))<1.0E-6
        if (sum(abs(magdiff))<1.0E-6):
            mag_symm_index.append(isymm)
    	    primed_index.append(delta_T)
            nsymm_mag = nsymm_mag + 1
    FL.close()
    nsymm_for_aver = nsymm_mag

    print " nsymm_for_aver ", nsymm_for_aver
    print " magsymop index ", mag_symm_index
    print " primed(-1),unprimed(+1): ", primed_index
    ...
    hr = HR.from_file_large(ndim, sdim,fwan)
...
#-------------- #-------------- #-------------- #-------------- #-------------- #--------------
    ...
    if ispinor:  # for spinor case, norbs=nband*2. 
        print "------------------spinfull--------------------------------"
        print "####### It is assumed up up dn dn orders of spin in wannier90_hr.dat (probably not the case for wannier90-3) ########"
        ...
        for rx in range(-sdim,sdim+1):
		    ...
		    if nsymm_mag>0:
		       # no Time inveration symmetrization to magnetic system
		       new = old*1.0
		    else:
...
# -------------------------------------------------------------------------------------
    ...
    print "I find the num of new blocks is:", sum(sum(sum(sum(sum(flags))))), "These new rpt blocks are listed in new_rpt_block.dat. Meanwhile, hoppings of these new blocks will be obtained from wannier90_hr.dat."
    ftmp=open("new_rpt_block.dat","w")
# -------------------------------------------------------------------------------------
            ...
            for rz in range(-sdim,sdim+1):
                ...
                for jatom in range(natom_wan):
                    ...
                    for iatom in range(natom_wan):
     		           ...
     		           print>>ftmp, "--new rpt block--", rx,ry,rz,iatom,jatom 
			   cnt = 0 
        ...
        for ry in range(-sdim,sdim+1):
			...
			chug = 0
				...
				# set this block zero ( alread set in init of hr_symmed.dat )
				    ...
				    cnt = cnt + 1
                                ...
                                proti = protmat[offi:offi+nori,offi:offi+nori,iptr,isymm] 
                                hr_ipjp= hr_mat[offip:offip+norip,offjp:offjp+norjp,izn,iyn,ixn]
                                if nsymm_mag>0 and isymm not in mag_symm_index: continue 
                                htmp = np.dot(np.dot(np.conj(np.transpose(proti)), hr_ipjp), protj)
                                if nsymm_mag>0 and primed_index[mag_symm_index.index(isymm)]==-1.0: 
                                    T_L = umata[offi:offi+nori,offi:offi+nori]
                                    T_R = umatb[offj:offj+norj,offj:offj+norj]
                                    htmp=np.conjugate(np.dot(np.dot(T_L,htmp),T_R)) # Time reversal update
                                hr_ij =  hr_ij + htmp
			...
			if cnt > 1 and cnt < nsymm*nptrans and sum(sum(abs(tmp)))>0.0: 
			   ...
			   sys.exit(0)
                        ...
                        hr_mat_symmed[offi:offi+nori,offj:offj+norj,izo,iyo,ixo] = hr_ij/np.float64(nsymm_for_aver*nptrans)
...
# dump hr_mat. this should also be symmed in r space.
    print "Step 3, dump new hr.dat..." # + " FM ? " + (nsymm_for_aver!=nsymm)
    today = datetime.date.today()
    dt = str(datetime.datetime.now())
    if delta_T==1.0: fsym='wannier90_hr.dat_unprimed_nsymm_'+str(nsymm_for_aver)
    if delta_T==-1.0: fsym='wannier90_hr.dat_primed_nsymm_'+str(nsymm_for_aver)
    ...
    if delta_T==1.0: 
        print "Please run step 2 as : python2 /dir/to/symmhr_addrptblock_magnetic_conjugate_when_primed.py -1.0 "+fsym
