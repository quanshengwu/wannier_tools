...
# For zhangwei, 2021/07/18
def get_orb_map_dz2(xp,yp,zp,ndim,orbi):
    if ndim!=2 or orbi!=5: 
       print "Error: local rot matrix for dz2 dx2-y2 orbital is of dim 2 !"
       sys.exit(0)
    ...
    for j in range(ndim):
        ...
        for st in fst:
            if st != xy: 
               ...
               t = t.subs(st,1)
        c4=t.evalf()
        if abs(c1)+abs(c2)+abs(c4)>1.0E-3: 
...
# new 2021-07-18
        ...
        ndim = 2
        orbi = 5
        rmat = get_orb_map_dz2(xp,yp,zp,ndim,orbi)
    elif case.strip() == 'f': 
        ndim = 7
        orbi = 3
