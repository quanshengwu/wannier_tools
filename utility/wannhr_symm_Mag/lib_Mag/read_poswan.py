...
class poswan():
    ...
    """
    def __init__(self, Amat, natom_pos, atoms_frac, atoms_cart, atoms_magmom, atoms_symbol, ispinor, natom_wan, type_orbs, num_orbs, atom_num, axis, nband, norbs):
        self.Amat=Amat
        self.natom_pos = natom_pos
        self.atoms_frac = atoms_frac
        self.atoms_cart = atoms_cart
        self.atoms_magmom = atoms_magmom
    ...
    def get_poswan(): 
        ...
        try: 
           with open(fname, 'r') as f:
               ...
               atoms_magmom=np.zeros((3,natom_pos),dtype=np.float64)
        ...
        try: 
            # read in wann.in
            with open(fname,'r') as f:
               orb_dim = {'s':1, 'p':3, 'd':5, 'f':7, 't2g':3, 'dz2':2}
               type_orbs = []
               num_orbs = []
               atom_num=[]
               nband = 0
        ...
        # read in magnetic moment for atoms according to OUTCAR
        print " reading the magnetic moment for each atom"
        fname = 'magmom.in'
        try: 
            # read in local axis
            with open(fname,'r') as f:
                ...
                if (nl!=0):# and nl ==natom_wan): 
                   for i in range(nl):
                      nt,sx,sy,sz = f.readline().strip().split() 
                      iw = np.int32(nt)-1
                      atoms_magmom[:,iw] = np.array([sx,sy,sz],dtype=np.float64)
                      print i, "magmom: \n", atoms_magmom[:,iw]
            return poswan(Amat, natom_pos, atoms_frac, atoms_cart, atoms_magmom, atoms_symbol, ispinor, natom_wan, type_orbs, num_orbs, atom_num, axis, nband, norbs)
        except IOError:
            print "File:" + "\"" + fname + "\"" +  " doesn't exist!"
