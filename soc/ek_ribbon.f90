...
! slab Bi2Se3
  ...
  subroutine ek_ribbon
     ...
     do i=1+cpuid, Nk1, num_cpu
        if (cpuid.eq.0) write(*, *) "Ribbonek the i'th kpoint", i, Nk1
        ...
        do j=1, Nslab1* Nslab2* Num_wann  !< bands
           do m=1, Nslab2
              do l=1, Num_wann  !< sum over orbitals
                 ...
                 + abs(CHamk(Num_wann*Nslab1*Nslab2 -(m-1)*Num_wann- l, j))**2 !& ! last slab
              enddo ! m
           enddo ! l
           ...
           do m=1, Nslab1
              do l=1, Num_wann  !< sum over orbitals
                 ...
                 + abs(CHamk(Num_wann*Nslab1*Nslab2 -(m-1)*Num_wann*Nslab2- l, j))**2 !& ! last slab
              enddo ! m
           enddo ! l
