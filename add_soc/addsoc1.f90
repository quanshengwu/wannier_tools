...
!> for d orbitals, the name of the orbital should be dxy, dyz, dxz, dx2-y2, dz2
!> the order of these orbitals can be specified by users
   subroutine  addsoc_magmom_all_2D
      ...
      allocate(soc_spd(LMMAX*2, LMMAX*2))
      allocate(H_soc(Num_wann_new, Num_wann_new))
      allocate(H_mag(Num_wann_new, Num_wann_new))
      allocate(start(Num_atoms))
      allocate(lambda(max_projs, Num_atoms))
      allocate(hx(max_projs, Num_atoms))
      allocate(hy(max_projs, Num_atoms))
      allocate(hz(max_projs, Num_atoms))
      allocate(orbital_index(max_projs, Num_atoms))
      ...
      do ia=1, Num_atoms
         ...
         do i=1, nprojs(ia)
            ...
            case ('px', 'Px', 'PX')
               orbital_index(i, ia)= 2
               lambda(i, ia)= lambda_p(it)/2d0
               hx(i, ia)= magmomx_p(ia)
               hy(i, ia)= magmomy_p(ia)
               hz(i, ia)= magmomz_p(ia)
            case ('py', 'Py', 'PY')
               orbital_index(i, ia)= 3
               lambda(i, ia)= lambda_p(it)/2d0
               hx(i, ia)= magmomx_p(ia)
            ...
            case ('dxy', 'Dxy', 'DXY')
               orbital_index(i, ia)= 5
               lambda(i, ia)= lambda_d(it)/2d0
               hx(i, ia)= magmomx_d(ia)
               hy(i, ia)= magmomy_d(ia)
               hz(i, ia)= magmomz_d(ia)
            case ('dyz', 'Dyz', 'DYZ')
               orbital_index(i, ia)= 6
               lambda(i, ia)= lambda_d(it)/2d0
               hx(i, ia)= magmomx_d(ia)
      ...
      do ia=1, Num_atoms
         do i=1, nprojs(ia)
            i1= start(ia)+ i- 1
            H_mag(i1, i1)= hz(i, ia)
            H_mag(i1+Num_wann, i1+Num_wann)= -hz(i, ia)
            H_mag(i1, i1+Num_wann)= hx(i, ia)- zi*hy(i, ia)
            H_mag(i1+Num_wann, i1)= hx(i, ia)+ zi*hy(i, ia)
         enddo ! i
      enddo ! ia
      ...
      do ir=1, nrpts
         if (irvec_2d(1, ir).ne.0 .or. irvec_2d(2, ir).ne.0) cycle
         HmnR(:, :, ir)= HmnR(:, :, ir)+ (H_soc+ H_mag)* ndegen(ir)
      enddo
      ...
      return
   end subroutine  addsoc_magmom_all_2D
...
!> for d orbitals, the name of the orbital should be dxy, dyz, dxz, dx2-y2, dz2
!> the order of these orbitals can be specified by users
   subroutine  addsoc_magmom_all_3D
      ...
      return
   end subroutine  addsoc_magmom_all_3D
