...
# hr, spin-full Hamr
    ndim = 23
    sdim = 11
    hr = HR.from_file_large(ndim, sdim,'hr_Hermit_TR_updnupdn.dat')
...
# rot matrix in terms of primitive cell basis
    ...
    rdege = np.zeros((ndim,ndim,ndim),dtype=np.int32)

    print "Now performing symmerizations, for details please refer symm.log"
    print "Step 1, thowing away blocks that will rot out of original rpts"
    f = open('symm.log', 'w')

    #print "fuck nsymm now 2"
    #nsymm = 2
    tenpct = np.int32(np.linspace(0,hr.nrpt,12))
# -------------------------------------------------------------------------------------
# Firstly, if one block <atom i | H {Rot_R} | atom j> is out of WS points set, let this
# block being zero
    irpt = 0
            ...
            for rz in range(-sdim,sdim+1):
                ...
                for jatom in range(natom):
                    ...
                    for iatom in range(natom):
                        offi=2*offs[iatom]
                        nori=2*nors[iatom]
                        # check block <atom i | H {Rot_R} | atom j>
                        flag = 0
                        for isymm in range(nsymm):         
                            ...
                            for iptr in range(nptrans):
                                ...
                                vec = new_rpt + atoms_frac[:,jatom]-atoms_frac[:,iatom] 
				dis = norm(np.dot(Amat,vec))
				#print isymm, iptr, iatom, jatom, "dis", dis
				if dis>20.0: flag=1
                                ...
                                if (abs(ixt)>sdim or abs(iyt)>sdim or abs(izt)>sdim): 
                                    print >>f, "--------------------Out of Huge Box, irtp", irpt, rpt, "-->", new_rpt, "-------------------"
                                    flag = 1
                                else:
                                    if hr_mat0[0,0,izn,iyn,ixn].real < -5000.0: # points out of hr.rpts
                                        print>>f, "------Out of hr.rpts, irtp ", irpt, rpt, "-->", new_rpt, "------", "iatom", iatom, "jatom", jatom
                                        flag = 1

                        if flag==1: # at least one block is out of hr.rpts: let that correponding block in H({rot_R}) being 0
                            for isymm in range(nsymm):         
                                rot = np.float64(symop[isymm][0])
                                rot_rpt = np.dot(rot,rpt) 
                                ...
                                for iptr in range(nptrans):
                                    ...
                                    ixn = ixt+sdim; iyn =iyt+sdim; izn = izt+sdim;
                                    if abs(ixt)>sdim or abs(iyt)>sdim or abs(izt)>sdim: 
                                        hr_mat[offi:offi+nori,offj:offj+norj,izo,iyo,ixo] = 0.0
                                    else: # for points in hr.rpts but one rot_R out of hr.rpts
                                        hr_mat[offip:offip+norip,offjp:offjp+norjp,izn,iyn,ixn] = 0.0
                        else:
                            print>>f, "-----block kept", "iatom", iatom, "jatom", jatom 
...
# determine whether new rpt     out of given rpts box
                                if abs(ixt)>sdim or abs(iyt)>sdim or abs(izt)>sdim: # for my case, ixyzt>10 may occur once in nsymm. so here should dnorm-1
                                    hr_mat_symmed[offi:offi+nori,offj:offj+norj,izo,iyo,ixo] = 0.0
                                    continue
